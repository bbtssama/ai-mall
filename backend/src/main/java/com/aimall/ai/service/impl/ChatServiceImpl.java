...
public class ChatServiceImpl implements ChatService {
    ...
    /**
     * 文本链路 system prompt：引导按需调用<b>双工具</b>，不编造。
     *
     * <p>V2 起模型手上有两只手：</p>
     * <ul>
     *   <li>{@code searchProduct} —— 价格/库存/找商品（结构化 → SQL）</li>
     *   <li>{@code searchDocs} —— 使用体验/功能细节/说明书（非结构化 → RAG）</li>
     * </ul>
     * system prompt 的职责是给"分工总原则"定调（实时数据查商品、体验问题查知识库），
     * 与两个 @Tool 的 description 形成"总-分"两级引导——双保险防模型选错工具。
     */
