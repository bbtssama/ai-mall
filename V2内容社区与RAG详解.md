...
**③ 向量为什么敢存 MySQL**——当前量级（500 笔记+100 商品 ≈ 数千切片）全量加载内存算余弦是毫秒级。上 pgvector/Milvus 的时机是"切片数上万"——检索层已抽象（EmbeddingClient/VectorRetriever），届时只换实现类。**在 100 商品上吹 Milvus 才是玩具行为**（[向量存 MySQL](V2内容社区与RAG详解【知识词典】.md#向量存-mysql)）。
...
一句话口径（面试金句）：**"我知道 ES 是搜索标配，但 500 篇笔记的量级 MySQL 全文索引毫秒级返回——为 500 条数据上 ES 集群才是过度设计。量级上来后检索层换实现，业务不动。"**（详见词典 [为什么不上 ES](V2内容社区与RAG详解【知识词典】.md#为什么不上-es)）
